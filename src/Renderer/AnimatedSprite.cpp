...
namespace Renderer
{
  ...
  void AnimatedSprite::update(const uint64_t delta)
  {
    ...
    {
        m_currentAnimaitonTime += delta;
        while(m_currentAnimaitonTime >= m_pCurrentAnimationDurations -> second[m_currentFrame].second)
        {
            m_currentAnimaitonTime -= m_pCurrentAnimationDurations -> second[m_currentFrame].second;
            ++m_currentFrame;
            m_dirty = true;
