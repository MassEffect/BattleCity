...
StartScreen::StartScreen(const std::vector<std::string>& startScreenDescription, Game* pGame)
                        ...
                        , m_tankSprite(std::make_pair(ResourceManager::getSprite("player1_yellow_tank_type1_sprite_right"),
                                                      glm::vec2(8 *  BLOCK_SIZE,
                                                                ...
                                                                6 * BLOCK_SIZE -
                                                                m_currentMenuSelection * 2 * BLOCK_SIZE
                                                               )
                                                     )
                                       )
...
void StartScreen::render() const
{
    ...
    m_menuSprite.first -> render(m_menuSprite.second, glm::vec2(MENU_WIDTH, MENU_HEIGHT), 0.0f);
    m_tankSprite.first -> render(glm::vec2(m_tankSprite.second.x, m_tankSprite.second.y - m_currentMenuSelection * 2 * BLOCK_SIZE), glm::vec2(TANK_SIZE), 0.0f, 0.0f, m_tankSpriteAnimator.getCurrentFrame());
}
